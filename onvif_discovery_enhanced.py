#!/usr/bin/env python3
"""
Enhanced ONVIF Device Discovery Tool

This script implements an enhanced version of ONVIF device discovery with additional
options to improve detection of cameras that might not respond to standard discovery.

Usage:
  python onvif_discovery_enhanced.py [--timeout SECONDS] [--json] [--scan-network] [--subnet SUBNET] [--ports PORTS]

Options:
  --timeout SECONDS    Set discovery timeout (default: 15 seconds)
  --json               Output results in JSON format
  --scan-network       Perform a network scan in addition to WS-Discovery
  --subnet SUBNET      Subnet to scan (default: 192.168.1.0/24)
  --ports PORTS        Comma-separated list of ports to check (default: 80,8000,8080)
"""
...
def check_onvif_device(ip, port):
    """Check if an IP:port combination is an ONVIF device by sending a GetSystemDateAndTime request"""
    try:
        # Create a simple GetSystemDateAndTime request
...
<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope">
  <s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
    <GetSystemDateAndTime xmlns="http://www.onvif.org/ver10/device/wsdl"/>
        ...
        # Try different possible ONVIF endpoints
        endpoints = [
            f"http://{ip}:{port}/onvif/device_service",
            f"http://{ip}:{port}/onvif/services",
            f"http://{ip}:{port}/onvif",
            f"http://{ip}:{port}/device_service"
        ]

        for endpoint in endpoints:
            try:
                response = requests.post(endpoint, data=soap_message, headers=headers, timeout=2)
                
                # Check if response contains ONVIF-related content
                if response.status_code == 200 and ('onvif' in response.text.lower() or 'getSystemDateAndTimeResponse' in response.text):
                    # Try to extract device information
                    name = "Unknown"
                    hardware = "Unknown"
                    
                    # Try to get device info from a GetDeviceInformation request
                    device_info_message = f"""<?xml version="1.0" encoding="UTF-8"?>
<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope">
  <s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
    <GetDeviceInformation xmlns="http://www.onvif.org/ver10/device/wsdl"/>
                    ...
                    try:
                        info_response = requests.post(endpoint, data=device_info_message, headers=headers, timeout=2)
                        
                        if info_response.status_code == 200:
                            # Try to extract manufacturer and model
                            manufacturer_match = re.search(r'<tds:Manufacturer>(.*?)</tds:Manufacturer>', info_response.text)
                            model_match = re.search(r'<tds:Model>(.*?)</tds:Model>', info_response.text)
                            
                            if manufacturer_match and model_match:
                                name = f"{manufacturer_match.group(1)} {model_match.group(1)}"
                                hardware = model_match.group(1)
                    except:
                        pass
...
def scan_network_for_onvif(subnet, ports, max_workers=20):
    """Scan a network subnet for ONVIF devices"""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for ip, port in active_hosts:
            futures.append(executor.submit(check_onvif_device, ip, port))
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced ONVIF Device Discovery Tool')
    ...
    if args.scan_network:
        ...
        network_devices = scan_network_for_onvif(subnet, args.ports)
